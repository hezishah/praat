...
namespace we {  // wave edit
  ...
  static int y2pt(int y) { return (y-bg_h)*20; }
  static int pt2y(int y) { return y/20 + bg_h; }
}
...
void static do_atexit() { puts("make_waves: bye!"); }
...
static void draw_wform(BgrWin *bgw,WavF *wf) { // supposed: bgw->parent = top_window
  bgw->clear();
  int i;
...
namespace we {
  ...
  static void move(BgrWin* bgw,int x,int y,int but) {
  ...
  static void up(BgrWin* bgw,int x,int y,int but) {
    state=eIdle;
...
static void waves_mode_cmd(RButtons*,int nr,int fire) {
  switch (nr) {
...
static void bgr_clear(BgrWin *bgr) { bgr->clear(); }
...
static void draw_curve(Render rend,Rect *r,int *buf,int dim,Uint32 color) {
...
static void wav_audio_callback(void*, Uint8 *stream, int len) {
...
namespace lfnoise {
  ...
  static float lp_section(const float fcut,float& d1,float& d2,const float white) {
    d2+=fcut*d1;                    // d2 = lowpass output
    float highpass=white-d2-qres*d1;
    d1+=fcut*highpass;              // d1 = bandpass output
    return d2;
...
static void bnc_audio_callback(void*, Uint8 *stream, int len) {
  bouncy->audio_callback(stream,len);
}

static void harm_audio_callback(void*, Uint8 *stream, int len) {
  harm->audio_callback(stream,len);
}

static void filt_audio_callback(void*, Uint8 *stream, int len) {
...
static int play_threadfun(void* data) {
...
static void right_arrow(Render rend,int par,int y_off) {
  trigonColor(rend,6,6,12,10,6,14,0x000000ff);
}

static void square(Render rend,int par,int y_off) {
  rectangleColor(rend,6,6,14,14,0xff0000ff);
}

static void play_cmd(Button* but) {
  if (i_am_playing) {
...
static void wave_out_cmd(Button* but) {
...
static void freqs_cmd(HVSlider *sl,int fire,bool rel) {
   const float
...
static void amval_cmd(HSlider *sl,int fire,bool rel) {
...
static void fmval_cmd(HSlider* sl,int fire,bool rel) {
...
static void mass_spring_cmd(HSlider* sl,int fire,bool rel) { // val 0 - 7
...
static void asym_cmd(HSlider *sl,int fire,bool rel) { // val 0 - 8
...
static int bouncy_threadfun(void* data) {
    (void)data;
...
static void extb_cmd(RExtButton* rb,bool is_act) {
  if (!is_act)
    return;
  switch (rb->id) {
    case 'wavs':
...
static void draw_bgw4(BgrWin *wb) {
  harm->print_info();
  wb->clear();
  for (int i=1;i<harm_max;++i) harm->draw_line(i);
}

static void draw_bgw5(BgrWin *bg) {
...
static void bouncy_down(BgrWin *bgw,int x,int y,int but) {
  int i,
      nearest = -1;
  bouncy->down=false;
  bouncy->selnode=0;
...
static void bouncy_moved(BgrWin *bgw,int x,int y,int but) {
...
static void bouncy_up(BgrWin *bgw,int x,int y,int but) {
  if (bouncy->selnode) {
    smooth::phase=1;
...
static void harm_down(BgrWin*,int x,int y,int but) {
  int amp,
      ind=harm->x2ind(x);
  HLine *lin;
  if (ind>0) {
...
static void test_filt_cmd(RButtons*,int nr,int fire) {
  filt->updating=true;
...
static bool set_config(FILE *conf) {  // no drawing!
  char buf[20];
  int i,
      n1,n2;
