...
_Label::_Label(const char* t):
  render_t(draw_ttf),
  draw_cmd(0),
  str(t),
  id(0) {
}
_Label::_Label(void (*dr)(Render,int,int y_off),int _id):
  render_t(draw_ttf),
  draw_cmd(dr),
  str(0),
  id(_id) {
}
_Label::_Label(const char *t,void (*dr)(Render,int,int y_off),int _id):
  render_t(draw_ttf),
  draw_cmd(dr),
  str(t),
  id(_id) {
...
void _Label::draw(Render rend,Point pnt) {
...
CheckBox::CheckBox(WinBase *pw,Style st,Rect rect,_Label lab,void (*_cmd)(CheckBox*)):
...
Button::Button(WinBase *pw,Style st,Rect rect,_Label lab,void (*_cmd)(Button*),const char *tit):
...
RButton* RButtons::add_rbut(_Label lab) {
  init_gui();
  int nr=next();
  but[nr].label=lab;
  RButton *rb=but+nr;
...
RExtButton::RExtButton(WinBase *pw,Style st,Rect rect,_Label lab,int _id):
...
RExtButton *ExtRButCtrl::add_extrbut(WinBase *pw,Rect rect,_Label lab,int id) {
  RExtButton *rb;
  ++butnr;
...
RButton *CmdMenu::add_mbut(_Label lab) {
...
static int send_ticks(void* data) {  // keep alive
  (void)data;
  while (!quit) {
