...
namespace scope {
  ...
  static void update(Sint16 *buffer,int i) {  // thread-safe
...
static void exec_info(int snr,bool do_draw) { // maybe in audio thread
...
static bool set_mass_spring(FILE *conf) {
  char buf[20];
  int n,
      n1,n2,n3,n4;
  float fl1;
...
static void write_note(FILE *conf,int col,int lnr,int& snr) {
  ScSection *sec=score->get_section(lnr,snr);
  bool play,
       stacc;
...
static bool write_config(FILE *conf) {
  int i;
  Node *n;
  Spring *s;
  Instrument *inst;
...
static void dial_cmd(const char* text,int cmd_id) {
  switch(cmd_id) {
    case 'w_cf': {
...
namespace smooth { // copy node location from bouncy to phys model
  ...
  Node b_nodes[NN],
       pm_nodes[NN];
  static void smooth(Instrument *instr) {
...
static void bouncy_down(BgrWin*,int x,int y,int but) {
  int i,
      nearest=-1,
      dx=0,dy;
...
static void bouncy_moved(BgrWin*,int x,int y,int but) {
...
static Bouncy *act_bnc() { return INST[act_instr]->bouncy; }
static PhysModel *act_phm() { return INST[act_instr]->phys_model; }

static void bouncy_up(BgrWin*,int x,int y,int but) {
...
static void extb_cmd(RExtButton *eb,bool is_act) {
  if (!is_act)
    return;
  switch (eb->id) {
    case 'mmas':
...
static void handle_key_event(SDL_Keysym *key,bool down) {
  switch (key->sym) {
    case SDLK_c:
    case SDLK_d:
    case SDLK_k:
...
static void extrb_label(Render rend,int nr,int) {
  const char *lab=0;
  switch (nr) {
    case 3:
...
static void draw_tfer_curve(BgrWin *tfc) {
  int i,
      x,
...
static void mouse_down(BgrWin *bgw,int x,int y,int but) {
  switch (bgw->id) {
...
static void mouse_moved(BgrWin *bgw,int x,int y,int but) {
  switch (bgw->id) {
    case 'scv': sv.mouse_moved(x,y,but); break;
  }
}

static void mouse_up(BgrWin *bgw,int x,int y,int but) {
  switch (bgw->id) {
...
static void draw_score(BgrWin *scv) {
  scv->clear();
...
static void draw_info(BgrWin *infv) {
  infv->clear();
  if (score->len-leftside<=0)
    return;
  int snr;
...
static void sv_scroll_cmd(HScrollbar* scb) {
...
static void fill_panel(Point top) {
...
int main(int argc,char** argv) {
  ...
  int ypos=10;
  eb_mmas=mode_ctr->add_extrbut(top_win,Rect(10,ypos,130,3*TDIST+2),_Label(extrb_label,3),'mmas');
  mode_ctr->act_lbut=eb_mmas;
  eb_edma=mode_ctr->add_extrbut(top_win,Rect(10,ypos+4+3*TDIST,130,3*TDIST+2),_Label(extrb_label,4),'edma');
  eb_edsp=mode_ctr->add_extrbut(top_win,Rect(10,ypos+8+7*TDIST,130,3*TDIST+2),_Label(extrb_label,5),'edsp');
  ypos+=11*TDIST;
  ...
  eb_sved=scv_edit_ctr->add_extrbut(top_win,Rect(190,ypos,130,3*TDIST+2),_Label(extrb_label,1),'sved');
  scv_edit_ctr->act_lbut=eb_sved;
  eb_ssel=scv_edit_ctr->add_extrbut(top_win,Rect(330,ypos,130,3*TDIST+2),_Label(extrb_label,2),'ssel');
